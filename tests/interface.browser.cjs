...
assert.equal(await page.locator('#settingsOpen').evaluate(el=>getComputedStyle(el).animationName),'action-blink','Configurar API deve ter animação ativa');
assert.equal(await page.locator('#settingsOpen').evaluate(async el=>{const animation=el.getAnimations()[0],start=animation?.currentTime;await new Promise(resolve=>setTimeout(resolve,120));return animation?.playState==='running'&&animation.currentTime>start}),true,'O pulso deve avançar no tempo, não ficar estático');
...
assert.equal(await page.locator('#pause').evaluate(el=>getComputedStyle(el).animationName),'action-blink','Pausar deve piscar durante a leitura');
const playingBoxes=await Promise.all(['#pause','#stop','.player-actions'].map(s=>page.locator(s).boundingBox()));assert.ok(playingBoxes.every(Boolean));
assert.ok(playingBoxes[1].width>=96,'Parar deve ter largura de botão completo');assert.ok(playingBoxes[1].x>=playingBoxes[0].x+playingBoxes[0].width,'Controles não devem se sobrepor');assert.ok(playingBoxes[1].x+playingBoxes[1].width<=playingBoxes[2].x+playingBoxes[2].width+.5,'Parar deve caber dentro da grade');
...
assert.equal(await page.locator('#pause').evaluate(el=>getComputedStyle(el).animationName),'none','Continuar não deve piscar quando pausado');
assert.equal(await page.locator('#stop').evaluate(el=>getComputedStyle(el).animationName),'action-blink','Parar deve piscar quando pausado');
...
await page.setViewportSize({width:280,height:800});assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth),true,'Controles devem caber em 280px');await page.screenshot({path:'test-results/ui-controls-280.png',fullPage:true});await page.setViewportSize({width:390,height:900});
